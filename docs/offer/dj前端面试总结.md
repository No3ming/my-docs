...
- react 最重要的性能问题：我当时回答了修改数据需要在 componentDidMount 中修改，避免初始化时多次 render，还有事件和对象不要直接在 render 中使用，采用变量引用事件 `<Demo onChange={() => {}}  />`
  // 避免这样写`。其实面试官是想问我 shouComponentupdate,将数据对比再决定要不要更新渲染。
...
2. 讲一下 vue 是如何进行双向绑定的以及 es6 给 vue 带来了那些改变。
...
6. typescript 为什么产生的带来了什么好处，在项目有有使用过么？
